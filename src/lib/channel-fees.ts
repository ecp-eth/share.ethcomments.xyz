...
export async function prepareContractAssetForTransfer({
  ...
  switch (tokenType) {
    ...
    case "erc1155": {
      ...
      // we decided to drop returning tokenId as it is niche case, so we don't support any
      // payment that requires tokenId atm.
      // (if we do want it back later we can add tokenId into metadata in the future)
